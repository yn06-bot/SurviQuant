...
with st.sidebar:
    ...
    st.caption(f"수익 {w['w_profit']*100:.0f}% / 방어 {w['w_defense']*100:.0f}%")
...
with ctrl3:
    sel_period_label = st.selectbox(
        ...
        index=len(PERIOD_OPTIONS) - 1,  # 전체 기본값
    )
...
with col_right:
    ...
    for col_obj, (label, value) in zip(snap_cols, [
        ("종가",       f"${latest['Close']:.2f}"),
        ("RSI",        f"{latest['RSI']:.1f}"),
        ("ADX",        f"{latest['ADX']:.1f}"),
        ("Volatility", f"{latest['Volatility']:.3f}"),
