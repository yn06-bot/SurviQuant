...
# ============================================================
# 4-D. 통합 차트 (과거 캔들+기술지표 → 미래 MC 밴드 하나로 연결)
# ============================================================
def build_unified_chart(ticker: str, ticker_df: pd.DataFrame,
                        volatility: float, period_days: int) -> go.Figure:
    """
    Row1: 과거 캔들 + MA20 + BB → 미래 MC 밴드 (x축 연속)
    Row2: RSI, Row3: ADX, Row4: Volatility
    """
    ...
    for y_val, color, label in [
        ...
        fig.add_annotation(x=1.01, y=y_val, xref="paper", yref="y",
            text=label, showarrow=False, font=dict(color=color, size=10), xanchor="left")
...
# ============================================================
with tab1:
    st.info("종목 상세 분석 탭을 이용해주세요. 이 탭은 추후 업데이트 예정입니다.")

# ============================================================
# 메인 — 종목 상세 분석 (단일 뷰, 2컬럼)
# ============================================================
with tab2:
    # 상단 컨트롤
...
> 각 지표의 임계값 기준으로 4가지 인사이트가 자동 생성됩니다.
    ...
    with col_right:
        ...
        # AI Score + 최신 지표 스냅샷 (차트 위)
        score_cols = st.columns([1.2, 1, 1, 1, 1, 1])
        with score_cols[0]:
            st.metric("🏆 AI Score", f"{ai_score:.1f} / 100",
                      help="수익 기여 + 방어 기여 합산")
        for col_obj, (label, value, tip) in zip(score_cols[1:], [
            ("종가",       f"${latest['Close']:.2f}",    None),
            ("RSI",        f"{latest['RSI']:.1f}",        "70↑ 과매수 / 30↓ 과매도"),
            ("ADX",        f"{latest['ADX']:.1f}",        "25↑ 추세 존재"),
        ...
        fig_bar.add_trace(go.Bar(x=[contrib_defense], y=[""], orientation="h",
            ...
            textfont=dict(color="white", size=12)))
        fig_bar.update_layout(barmode="stack", height=60,
            margin=dict(l=0, r=0, t=4, b=4),
            xaxis=dict(range=[0, 100], showticklabels=False, showgrid=False),
            yaxis=dict(showticklabels=False),
            legend=dict(orientation="h", y=1.8, x=0), template="plotly_white")
        ...
        # 통합 차트 (기술지표 → 미래 예측 이어짐)
        chart_title_col, chart_info_col = st.columns([5, 1])
        with chart_title_col:
            st.markdown("#### 📊 기술지표 + 🔮 미래 가격 시나리오")
        with chart_info_col:
            with st.popover("ℹ️"):
                st.markdown("**통합 차트 안내**")
                st.markdown("""
**과거 구간** (캔들차트)
- 🟢🔴 캔들: 실제 주가 OHLC
...
**미래 구간** (🟡 점선 이후)
- 연보라 밴드: 5~95% 시나리오 (90% 경로 포함)
- 진보라 밴드: 25~75% 시나리오 (50% 경로 포함)
...
**RSI** — 70↑ 과매수 / 30↓ 과매도
**ADX** — 25↑ 추세 존재 / 미만 횡보
**Volatility** — SurviQuant 핵심 변수 (HR 손실 5.28)

> 미래 구간은 GBM 500경로 통계 시나리오입니다.
                """)

        if not YF_AVAILABLE:
            st.caption("*(yfinance 미설치 — 캐시 데이터 기준 시뮬레이션)*")
        ...
        st.caption("⚠️ 미래 예측은 통계적 시나리오이며 투자 권유가 아닙니다.")
# ============================================================
# 8. 푸터
# ============================================================
st.divider()
