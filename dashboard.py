...
PERSONA_PRESETS = {
    "conservative": {"label": "안정형", "w_profit": 0.31, "w_defense": 0.69, "lambda": 2.25,
                     "desc": "손실 회피 우선 — Kahneman λ=2.25 적용"},
...
def build_unified_chart(ticker, ticker_df, volatility, period_days):
    ...
    fig.add_trace(go.Scatter(x=hist["Date"], y=hist["ADX"], name="ADX",
    ...
    fig.add_trace(go.Scatter(x=hist["Date"], y=hist["Volatility"], name="Volatility",
...
with st.sidebar:
    ...
    st.caption(f"{w['desc']}")
    st.caption(f"수익 가중치 **{w['w_profit']:.2f}** / 방어 가중치 **{w['w_defense']:.2f}** (λ={w['lambda']})")
    ...
    st.caption(f"기준일: **{meta.get('reference_date', '-')}**")
    st.caption(f"신뢰도 (C-index)")
    st.caption(f"— 수익 모델: **{c_index.get('profit_model', '-')}**")
    st.caption(f"— 손실 모델: **{c_index.get('loss_model', '-')}**")
    st.divider()
...
with title_col:
    st.title("SurviQuant")
    st.caption("생존분석 기반 S&P 500 AI 투자 대시보드 — 단순 방향성이 아닌 '도달 확률 + 시점'을 정량 제공")
with info_col:
...
- RSF 추론 대상: 50개 (5섹터 대표주)
        ...
        st.markdown("""
Tversky & Kahneman (1992) Prospect Theory 손실 회피 계수 λ ≈ 2.25를 기반으로 설계하였습니다.
...
- 공격형: 69/31 (λ=0.44) — 고수익 추구
        ...
        st.markdown("""
코어 규칙 유지 + 도메인 규칙 교체만으로 KOSPI, ETF, 섹터 심화 분석 모두 적용 가능한 모듈형 구조입니다.
        """)
...
st.markdown(
    f"### {sel_ticker} &nbsp;"
    f"<span style='background:{signal_color};color:white;padding:4px 14px;"
    f"border-radius:16px;font-size:14px;font-weight:600;'>{signal_label}</span>",
    unsafe_allow_html=True)
st.caption(f"{row['Sector']}  |  성향: {w_preset['label']}")
st.divider()
...
with col_left:
    ...
    with surv_h:
        st.markdown("#### 생존분석 — 수익·손실 도달 확률 추이")
    with surv_i:
        with st.popover("ⓘ"):
...
실제 RSF 출력값이 아닌 종점 확률 기반 근사치입니다.
    ...
    fig_surv = go.Figure()
    fig_surv.add_trace(go.Scatter(x=days, y=cum_p, name="수익 도달 (+10%)",
        line=dict(color="#10B981", width=2.5),
        fill="tozeroy", fillcolor="rgba(16,185,129,0.08)",
        hovertemplate="Day %{x}: %{y:.1f}%<extra>수익 도달</extra>"))
    fig_surv.add_trace(go.Scatter(x=days, y=cum_l, name="손실 도달 (-10%)",
    ...
    fig_surv.add_annotation(x=20, y=1, xref="x", yref="paper",
        ...
        font=dict(color="#9CA3AF", size=10), xanchor="right", yanchor="top")
...
- MA20: 단기 추세 위치 확인 (기준: ±5%)
    ...
    insights = generate_insights(
        ...
        float(latest["Volatility"]), float(latest["Close"]), float(latest["MA20"]))
    for label, message in insights:
        st.markdown(
            f"""<div style="background:#F8FAFC;border-left:4px solid #3B82F6;
                ...
                <div style="font-size:12px;color:#6B7280;margin-top:3px;">{message}</div>
...
with col_right:
    ...
    for col_obj, (label, value, tip) in zip(snap_cols, [
        ("종가",       f"${latest['Close']:.2f}",    None),
        ("RSI",        f"{latest['RSI']:.1f}",        "70 이상 과매수 / 30 이하 과매도"),
        ("ADX",        f"{latest['ADX']:.1f}",        "25 이상이면 추세 존재"),
        ("Volatility", f"{latest['Volatility']:.3f}", "핵심 예측 변수 (HR 손실 5.28)"),
        ("MA20",       f"${latest['MA20']:.2f}",      "20일 이동평균"),
    ]):
        with col_obj:
    ...
    # 종목 차트 (통합)
    chart_h, chart_i = st.columns([5, 1])
    with chart_h:
        st.markdown("#### 종목 차트")
    with chart_i:
        with st.popover("ⓘ"):
            st.markdown("**차트 구성 안내**")
            st.markdown("""
**과거 구간** (캔들차트)
- MA20: 20일 이동평균 — 위에 있으면 단기 상승 추세
- 볼린저 밴드: 상단 돌파 → 과매수, 하단 이탈 → 과매도

**미래 구간** (노란 점선 이후, 몬테카를로 500경로)
- 연보라 밴드: 5~95% 시나리오
- 진보라 밴드: 25~75% 시나리오
...
미래 구간은 통계적 시나리오입니다.
    ...
    with sc1:
        st.metric("AI Score", f"{ai_score:.1f} / 100",
                  help="수익 기여 + 방어 기여의 합산 점수 (0~100)")
    with sc2:
        st.metric("수익 기여", f"{contrib_profit:.1f}점",
                  f"Profit {row['Profit_Chance']:.1f}% × {w_preset['w_profit']:.2f}")
    with sc3:
        st.metric("방어 기여", f"{contrib_defense:.1f}점",
                  f"Defense {100-row['Loss_Risk']:.1f}% × {w_preset['w_defense']:.2f}")

    fig_bar = go.Figure()
    fig_bar.add_trace(go.Bar(x=[contrib_profit], y=[""], orientation="h",
        name="수익 기여", marker_color="#10B981",
        text=[f"수익 {contrib_profit:.1f}점"], textposition="inside",
        textfont=dict(color="white", size=12)))
    fig_bar.add_trace(go.Bar(x=[contrib_defense], y=[""], orientation="h",
        name="방어 기여", marker_color="#3B82F6",
        text=[f"방어 {contrib_defense:.1f}점"], textposition="inside",
        textfont=dict(color="white", size=12)))
    fig_bar.update_layout(
        ...
        legend=dict(orientation="h", y=1.8, x=0),
        template="plotly_white")
    st.plotly_chart(fig_bar, use_container_width=True)

    st.caption(
        f"AI Score = Profit_Chance × {w_preset['w_profit']:.2f} "
        f"+ (100 − Loss_Risk) × {w_preset['w_defense']:.2f}  |  "
        f"성향: {w_preset['label']} (λ={w_preset['lambda']})"
    )
...
# ============================================================
st.divider()
st.caption("본 대시보드는 학술·시연 목적으로 제작되었습니다. 투자 권유 또는 자문이 아닙니다.")
