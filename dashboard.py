...
실제 RSF 출력값이 아닌 종점 확률 기반 근사치입니다.
        ...
        fig_surv.update_layout(
            height=260, margin=dict(l=0, r=10, t=8, b=0),
...
with st.container(border=True):
    ...
    # 수식 설명
    st.markdown(
        f"<div style='margin-top:10px;color:#6B7280;font-size:13px;'>"
        f"Profit {row['Profit_Chance']:.1f}% × {w_preset['w_profit']:.2f}"
        f"&nbsp;+&nbsp;"
        f"Defense {100 - row['Loss_Risk']:.1f}% × {w_preset['w_defense']:.2f}"
        f"&nbsp;=&nbsp;<strong style='color:#111;font-size:14px;'>{ai_score:.1f}</strong>"
        f"</div>",
        unsafe_allow_html=True)
